-- Implementations of quicksort using sequences under the hood.

module QuickSeq (sort, sort') where
...
sortSeq (p S.:< rest) = sortSeq lessers S.>< S.singleton p S.>< sortSeq greaters
